...
def parse_args():
    ...
    parser.add_argument("--output", type=str, default="pusher_wall_alpha.csv", help="Output CSV file name")
...
if __name__ == "__main__":
    ...
    total_iterations = len([False, True]) * len(args.test_seeds) * args.repeats
    total_steps = total_iterations * args.eval_steps

    print(f"Starting evaluation:")
    print(f"  - Wall configs: {len([False, True])}")
    ...
    with tqdm(total=total_iterations, desc="Evaluating") as pbar:
        for wall_idx, wall in enumerate([False, True]): #[0.1,0.3,0.5]
            pbar.set_description(f"Testing {'with wall' if wall else 'without wall'}")
            if wall:
            ...
            # Load model once per wall configuration (not per seed)
            ...
            for seed_idx, seed in enumerate(args.test_seeds):
                ...
                for k in range(args.repeats):
                    ...
                    # Update progress bar
                    pbar.update(1)
                    episodes_completed = (wall_idx * len(args.test_seeds) * args.repeats +
                                         seed_idx * args.repeats + (k + 1))
                    steps_completed = episodes_completed * args.eval_steps
                    pbar.set_postfix({'seed': seed, 'wall': 'Yes' if wall else 'No',
                ...
                df.loc[len(df)] = [model_name, seed, avg_reward, avg_distance, wall, avg_success]
