...
class PusherEnv_rewardshift(MujocoEnv, utils.EzPickle):
    r"""
    ## Description
    ...
    * v0: Initial versions release.
    ...
    def __init__(
        self,
        xml_file: str = os.path.join(os.path.dirname(__file__), "pusher_v5.xml"),
        frame_skip: int = 5,
        default_camera_config: Dict[str, Union[float, int]] = DEFAULT_CAMERA_CONFIG,
        reward_near_weight: float = 0.5,
        reward_dist_weight: float = 1,
        reward_control_weight: float = 0.1,
        success_truncation: bool = True,
        radius: float = 0.05,
        **kwargs,
    ):
        ...
        self.success_truncation = success_truncation
        self.radius = radius  # radius of the circle on which the hidden goal is sampled
    ...
    def step(self, action):
        ...
        if info["reward_dist"] > -0.1:
            if self.success_truncation:
                done = True
            info["success"] = True
    ...
    def _get_rew(self, action):
        ...
        vec_2 = self.get_body_com("object") - self.hidden_goal_pos
    ...
    def reset_model(self):
        ...
        #         break
        self.cylinder_pos = np.array([-0.3, 0.2])
        qpos[-4:-2] = self.cylinder_pos
        qpos[-2:] = self.goal_pos
        qvel = self.init_qvel + self.np_random.uniform( low=-0.005, high=0.005, size=self.model.nv)
        qvel[-4:] = 0
        self.set_state(qpos, qvel)
        self.hidden_goal_pos = self.sample_on_circle(self.radius)
    ...
    def sample_on_circle(self, r):
        ...
        point : ndarray of shape (2,)
            A point (x, y) on the circle's circumference.
        """
        cx, cy = self.get_body_com("goal")[:2]
        theta = np.random.uniform(0, 2 * np.pi)          # random angle
        x = cx + r * np.cos(theta)
        y = cy + r * np.sin(theta)
        return np.array([x, y, self.get_body_com("goal")[2]])  # keep z-coordinate same as goal's z-coordinate


gymnasium.register(
    id="CustomPusher_rewardshift-v1",
    entry_point="mujoco_local.Pusher_v5_reward_shift:PusherEnv_rewardshift",
    )  